...
## Ghi chú nghiên cứu: Bellman operator và contraction

Nếu dạy cho học viên research, Bellman equation không nên chỉ được xem như một công thức đệ quy. Cách nhìn sâu hơn là xem Bellman như một operator tác động lên không gian các value functions.

Với một policy cố định $\pi$, Bellman expectation operator được viết:

$$
(T^\pi V)(s) = \sum_a \pi(a \mid s) \sum_{s', r} p(s', r \mid s, a) [r + \gamma V(s')]
$$

Điểm quan trọng của công thức không nằm ở ký hiệu, mà ở việc $T^\pi$ nhận một hàm value bất kỳ $V$ rồi tạo ra một hàm value mới tốt hơn theo phương trình một bước. Value thật của policy, $V^\pi$, là fixed point của operator này:

$$
V^\pi = T^\pi V^\pi
$$

Với finite MDP và $0 \leq \gamma < 1$, $T^\pi$ là một contraction trong chuẩn vô cực:

$$
\|T^\pi V - T^\pi U\|_\infty \leq \gamma \|V - U\|_\infty
$$

Đọc theo nghĩa đời thường: nếu hai ước lượng value khác nhau một lượng nào đó, sau khi cùng đi qua Bellman operator, khoảng cách giữa chúng bị co lại ít nhất theo hệ số $\gamma$. Đây là lý do value iteration có nền tảng toán học vững: lặp operator sẽ tiến tới fixed point duy nhất.

Bellman optimality operator tương tự nhưng thay kỳ vọng theo policy bằng phép tối ưu:

$$
(T^* V)(s) = \max_a \sum_{s', r} p(s', r \mid s, a) [r + \gamma V(s')]
$$

Với điều kiện tabular, finite state-action space và discount nhỏ hơn 1, $T^*$ cũng là contraction. Fixed point của nó là $V^*$. Từ $V^*$ hoặc $Q^*$, ta lấy greedy policy để thu được policy tối ưu.

Điểm này rất quan trọng khi nối sang deep RL. Trong tabular RL, operator có thể có contraction rõ ràng. Khi dùng neural network, ta không còn lặp trực tiếp trên toàn bộ không gian value functions nữa, mà lặp trên một họ hàm bị giới hạn bởi tham số $\theta$. Khi đó, convergence guarantee cổ điển không tự động còn đúng. Đây là một trong những lý do function approximation làm RL khó hơn supervised learning.

Với LLM agent, ta thường còn xa hơn finite MDP: state là context dài, environment không stationarity, reward là proxy, và transition có thể phụ thuộc vào user, tool, policy update, prompt template. Vì vậy, Bellman operator vẫn là ngôn ngữ tư duy rất mạnh, nhưng mọi kết luận toán học cần được đọc kèm giả định.
